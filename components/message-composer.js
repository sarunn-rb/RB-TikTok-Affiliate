...
export function MessageComposer({ connected, initialCreator = "", initialUsername = "" }) {
  ...
  async function createConversation() {
    ...
    setPending("conversation"); setError(""); setResult(null);
  ...
  async function sendMessage(event) {
    ...
    setPending("message"); setError(""); setResult(null);
  ...
  return (
    <div className="message-layout">
      ...
      <form className="panel panel-pad stack" onSubmit={sendMessage}>
        <div><h2 className="panel-title">2. Send affiliate message</h2><p className="panel-copy">The message is sent only to the selected creator.</p></div>
      ...
      {(error || result) && <div className={`notice ${error ? "notice-error" : "notice-success"}`} role="status">{result && <CheckCircle2 size={18} />}{error || <span>{result.text}{result.messageId && <> Message ID: <code>{result.messageId}</code>.</>}{result.requestId && <> Request ID: <code>{result.requestId}</code>.</>}</span>}</div>}
    </div>
  );
}
